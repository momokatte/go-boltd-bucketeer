...
*/
func NewPath(buckets ...[]byte) (p Path) {
...
/*
NewStringPath creates a new Path from one or more bucket names.
*/
func NewStringPath(buckets ...string) (p Path) {
	p = make(Path, len(buckets))
...
*/
func (p Path) Nest(bucket []byte) (newPath Path) {
	newPath = make(Path, len(p)+1)
	copy(newPath, p)
	newPath[len(p)] = bucket
	return
}
